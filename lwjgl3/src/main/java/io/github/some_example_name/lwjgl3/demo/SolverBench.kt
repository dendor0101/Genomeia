 ...
 *
 * =====================================================================================
 *  ЭТОТ СТЕНД ОБЯЗАН БЫТЬ ПЕРЕЗАПУЩЕН НА ANDROID.
 * =====================================================================================
 *
 * Все цифры в комментариях проекта — десктопные (16 ядер, JDK 21). Для решения «double
 * или float» этого НЕ ДОСТАТОЧНО, потому что double удваивает рабочий набор, а на
 * мобильных лишний трафик в память стоит дороже по трём причинам:
 *
 *   - последний уровень кэша меньше и ДЕЛИТСЯ С GPU (SLC 4..8 МБ против 16..32 МБ L3).
 *     Обратите внимание: L1 при этом не меньше, у Apple с A14 он 128 КБ против типичных
 *     32..48 КБ на x86. Проблема не в «кэши меньше вообще», а в последнем уровне и в
 *     том, что за него конкурирует отрисовка;
 *   - LPDDR уже десктопной памяти, предвыборка слабее, меньше промахов держится в
 *     полёте — неаккуратный доступ прощается хуже;
 *   - обращение к DRAM дорого по энергии, а устойчивая производительность ограничена
 *     троттлингом: лишний трафик бьёт не только по мгновенному FPS, но и по тому,
 *     сколько он держится до сброса частот.
 *
 * ЧТО СМОТРЕТЬ В ТАБЛИЦЕ. Не одно число, а ГДЕ отношение double/float начинает расти —
 * это и есть точка, в которой рабочий набор перестал помещаться. На десктопе она видна
 * на 1024 копиях (1.23x против ровных 1.14x в середине). На телефоне она наступит
 * раньше и может быть круче; столбцы «КБ f» и «КБ d» дают привязку к размерам кэшей
 * конкретного устройства.
 *
 * КАК ЗАПУСТИТЬ. Через :lwjgl3 не выйдет — нужна задача в модуле android. Класс сюда
 * годится как есть: он не трогает ни gdx, ни файловую систему сверх чтения тела,
 * а Topology грузится тем же BodyFile.
 *
 * ЕСЛИ ВЫЙДЕТ СИЛЬНО ХУЖЕ 15%, вернитесь к разобранным альтернативам: смешанная
 * точность (замерена, экономии не даёт), эмуляция двойной мантиссы на float (1.43x,
 * памяти столько же) и фиксированная точка int64 (утечка ровно ноль, 2.93x на
 * десктопе). Последняя тем интереснее, чем дороже окажется double: разрыв между ними
 * сократится. Все три замера — в комментариях RealBodyDemo и в стенде PrecisionScale.
 */
...
fun main(args: Array<String>) {
    ...
    println()
    println("Столбцы КБ — горячее состояние целиком. Ориентиры на ДЕСКТОПЕ: L1d 32..48 КБ")
    println("на ядро, L2 0.5..2 МБ, L3 общий 16..32 МБ. Отношение double/float надо читать")
    println("вместе с ними: пока набор в кэше, разницы почти нет, а когда вываливается в")
    println("память, счёт идёт по БАЙТАМ, и double платит вдвое.")
    println()
    println("НА ANDROID ЭТО НАДО ПЕРЕЗАПУСТИТЬ. Там последний уровень кэша 4..8 МБ и делится")
    println("с GPU, память уже, предвыборка слабее, а троттлинг наказывает за трафик. Точка,")
    println("где отношение начинает расти, наступит РАНЬШЕ показанной здесь. Подробности и")
    println("что делать, если выйдет сильно хуже 15%, — в шапке этого файла.")
    println()
}
