...
fun main(args: Array<String>) {
    ...
    // РАЗРЫВ ПО УМОЛЧАНИЮ ВЫКЛЮЧЕН, и включается только там, где проверяется он сам.
    //
    // Причина не в удобстве. Почти все проверки здесь мерят СОХРАНЕНИЕ: затухание
    // импульса, остаточное вращение, утечку стадий. Рвущаяся ткань меняет топологию
    // прямо посреди замера — число связей, контур, число организмов, — и величина
    // перестаёт относиться к одному и тому же телу. Замер после распрямления складки
    // с включённым разрывом показывал длиннейшее граничное ребро 7.37 при обычном
    // 0.03: тело было изодрано в клочья, и «затухание импульса» считалось уже по ним.
    //
    // Плюс SwimSolver разрыва не умеет вовсе — он для подбора параметров плавания,
    // где ткань цела по условию. С включённым разрывом побитовое сравнение с ним
    // расходится законно, и проверка ловила бы не ошибку, а разницу задач.
    P.setTearing(false)
    ...
    run {
        // ЗДЕСЬ разрыв включён: проверяется именно он. Ткань обязана рваться под
        // ударом, иначе вопрос «может ли тело войти в мягкую ткань без разрыва»
        // остаётся без ответа — вечные связи выдержат что угодно.
        P.setTearing(true)
        P.resetState(); P.resetCounters()
        ...
        if (order.size < 2 || P.organismSize(order[1]) < 2) {
            ...
            val killed = P.killedLinks()
            println(String.format(Locale.ROOT,
                "       таран на %.1f клеток/тик: проникновение %.3f, помечено %d, УБИТО связей %d из %d, потолок %d раз, пик %.1f",
                speed * dt / topo.meanLinkLength, worstPen, P.tornCount(), killed, topo.conCount,
                P.speedCapHits(), P.peakSpeedCellsPerTick()))
            // ГЛАВНЫЙ ВОПРОС, ради которого разрыв и сделан: может ли тело войти в
            // мягкую ткань, НЕ порвав её. Пока связи были вечными, ответ получить
            // было нельзя — ткань обязана была выдержать что угодно.
            //
            // Инвариант: глубокое проникновение допустимо ТОЛЬКО там, где ткань
            // порвалась. Проникновение без единого разрыва означает, что контур
            // дырявый, а не перегруженный.
            check("глубокое проникновение только через разрыв",
                "тело вошло в мягкую ткань, не порвав её — контур дырявый") {
                ok = worstPen < 0.5 || killed > 0
                detail = String.format(Locale.ROOT,
                    "проникновение %.3f, убито связей %d", worstPen, killed)
            }
