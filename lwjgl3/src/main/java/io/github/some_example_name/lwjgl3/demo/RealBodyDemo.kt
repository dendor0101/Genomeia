 ...
 *                        затронутого.
 *      RCM-порядок       НЕ портится ростом, вопреки первому впечатлению. Cuthill-McKee
 *                        это обход в ширину от периферийной вершины, а рост организма
 *                        от первой клетки — тоже обход в ширину. Значит если новые
 *                        клетки дописываются в порядке роста, порядок получается сам
 *                        собой близким к тому, что выдал бы RCM: он не сбивается, он
 *                        воспроизводится тем же механизмом.
 *
 *                        Портится другое, и лечится оно тоже другим:
 *                          * РАЗНОВРЕМЕННЫЕ ВЕТВИ. Хвост, выросший поздно, крепится к
 *                            клетке торса, добавленной рано, и эта связь длинная. Но
 *                            она ОДНА НА ВЕТВЬ, а не на клетку, и не накапливается.
 *                          * ОКНА ОТ ПОГИБШИХ КЛЕТОК. Они не ломают ПОРЯДОК, они ломают
 *                            ПЛОТНОСТЬ: в кэш-линию влезает 8 double, и если три из них
 *                            мёртвые, 37% линии загружено впустую.
 *
 *                        Отсюда: чинить надо не порядок, а плотность. УПЛОТНЕНИЕ
 *                        закрывает окна и сохраняет порядок — оно дешевле полной
 *                        пересортировки и лечит ровно то, что действительно портится.
 *                        Пересортировку гонять, только если ЗАМЕРЕНО, что ширина ленты
 *                        выросла; это измеримо, гадать незачем.
 *
 *                        Оговорка: всё сказанное верно, пока новая клетка дописывается
 *                        в блок СВОЕГО организма. Если её класть в конец общей арены,
 *                        локальность действительно рушится — там связь пойдёт через
 *                        всю арену.
 *
